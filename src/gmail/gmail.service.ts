...
export class GmailService {
  ...
  constructor() {
    this.transporter = nodemailer.createTransport({
      ...
      auth: {
        user: process.env.GMAIL_USER,     // tu cuenta @gmail.com o Workspace
        pass: process.env.GMAIL_APP_PASS, // App Password (con 2FA activado)
      },
    });
  }

  async sendPasswordReset(to: string, name: string, link: string) {
    const from = process.env.MAIL_FROM || `SkyNet Visitas <${process.env.GMAIL_USER}>`;
    ...
    return `
      <div style="background:#f6f7fb;padding:24px;font-family:Arial,sans-serif;color:#111;">
        <table width="100%" style="max-width:620px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,.06)">
          <tr><td style="padding:24px;text-align:center;font-weight:700;font-size:20px">SkyNet Visitas</td></tr>
          <tr><td style="padding:0 24px"><hr style="border:none;border-top:1px solid #eef2f7"></td></tr>
          <tr><td style="padding:24px">
            <p style="font-size:16px;margin:0 0 12px">Hola <strong>${this.escape(name||'usuario')}</strong>,</p>
            ...
            <p style="font-size:12px;color:#6b7280;margin:0 0 8px">Si el botón no funciona, copia este enlace:</p>
            <p style="word-break:break-all;font-size:12px"><a href="${link}" style="color:#1d4ed8;text-decoration:none">${link}</a></p>
          </td></tr>
          <tr><td style="background:#f9fafb;padding:16px;text-align:center;color:#6b7280;font-size:12px">
